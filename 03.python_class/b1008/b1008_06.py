...
# students.sort(key=lambda x:x[5]) #순차정렬 (합계순)
# students.sort(key=lambda x:x[-5]) #역순정렬 (합계순)
#students.sort(key=lambda x:x[1]) #순차정렬 (이름순) ##중요
#students.sort(key=lambda x:x[1],reverse=True) #역순정렬(이름순)
